...
select source.getNode().asExpr().getFile().getAbsolutePath()+"$$"+source.getNode().asExpr().getFile().getLocation().getStartLine() ,source, source, sink, "This string literal that is $@ contains " + report, sink,
