...
where
  ...
  c2.getValue().toInt() != 0
select guard.getLocation().getFile().getAbsolutePath()+"$$"+guard.getLocation().getStartLine() ,guard, "This expression conflates OK and non-OK results from $@.", source, source.toString()
