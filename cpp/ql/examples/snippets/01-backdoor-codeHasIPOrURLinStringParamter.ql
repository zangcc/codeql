...
where fc.getAnArgument() = sl and  sl.getValue().regexpMatch(domainOrIpRegex())  and loc1 = sl.getLocation().getFile().getAbsolutePath() +"$$" + sl.getLocation().getStartLine()+ " 代码中出现了域名或者IP,请手工检查是否可能存在后门：" + sl.getValue().toString() 
