...
where
  ...
  and exists(int i , string s |
    ...
    s.regexpMatch(".*(key|password|secret|mnemonic|mnemo|memno|plain|pair|vault|arn|param|token|content|mail|ldap|record|response|request|result|account|conf|auth).*")  and
    str1 = s
    )
