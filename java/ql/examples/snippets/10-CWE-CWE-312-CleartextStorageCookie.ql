...
where
  ...
  data.flowsTo(input)
select store,store.getLocation().getFile().getAbsolutePath()+"$$"+store.getLocation().getStartLine()+"-"+ store.getLocation().getEndLine(), "This stores cookie $@ containing $@ which was $@.", s, s.toString(), data,
