...
where
  ...
  data.flowsTo(input)
select store.getLocation().getFile().getAbsolutePath()+"$$"+store.getLocation().getStartLine() ,store, "This stores the local file $@ containing $@ which was $@.", s, s.toString(), data,
