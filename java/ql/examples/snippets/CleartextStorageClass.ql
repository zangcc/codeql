...
where
  ...
  data.flowsTo(input)
select store.getLocation().getFile().getAbsolutePath()+"$$"+store.getLocation().getStartLine() ,store, "This stores the storable class $@ containing $@ which was $@.", s, s.toString(),
