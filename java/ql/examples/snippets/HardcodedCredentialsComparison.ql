...
select source.getNode().asExpr().getFile().getAbsolutePath()+"$$"+source.getNode().asExpr().getFile().getLocation().getStartLine() ,source, "Hard-coded value is $@ with password variable $@.", sink, "compared", p, p.getName()
