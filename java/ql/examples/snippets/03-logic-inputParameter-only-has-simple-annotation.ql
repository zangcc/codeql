...
where m.hasAnnotation("org.springframework.web.bind.annotation","PostMapping")
  ...
  and onlyHasSpecialAnotaion(filed)= "True"
...
bindingset[field]
string onlyHasSpecialAnotaion(Field field) {
  field.getAnAnnotation().getType().hasName(["NotBlank", "NotNull", "Valid", "NotEmpty"])
  and not exists(Annotation b | b.getAnnotatedElement() = field.getAnAnnotation().getAnnotatedElement() and not b.getType().hasName(["NotBlank", "NotNull", "Valid", "NotEmpty"]))
  and result = "True"
  or not exists( Annotation an | an.getAnnotatedElement() = field.getAnAnnotation().getAnnotatedElement()) 
  and result = "True"
}
