...
where
  ...
  data.flowsTo(input)
select store,store.getLocation().getFile().getAbsolutePath()+":"+store.getLocation().getStartLine()+"-"+ store.getLocation().getEndLine(), "This stores the 'SharedPreferences' class $@ containing $@ which $@.", s,
