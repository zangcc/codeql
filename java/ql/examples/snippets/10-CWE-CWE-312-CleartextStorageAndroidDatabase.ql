...
where
  ...
  data.flowsTo(input)
select store,store.getLocation().getFile().getAbsolutePath()+"$$"+store.getLocation().getStartLine()+"-"+ store.getLocation().getEndLine(), "This stores data in a SQLite database $@ containing $@ which was $@.", s,
