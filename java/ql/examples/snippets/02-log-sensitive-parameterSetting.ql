...
where ma.getQualifier() = v.getAnAccess()
  ...
  and v.getName().toString().regexpMatch("(?i).*(key|password|secret|mnemonic|mnemo|memno|plain|pair|vault|arn|param|token|content|mail|ldap|record|response|request|result|account|conf|auth).*")
