...
select sink.getNode(), source, sink,sink.getNode().getLocation().getFile().getAbsolutePath()+"$$"+sink.getNode().getLocation().getStartLine(), "Insecure basic authentication from a $@.", source.getNode(),
  "HTTP URL"
