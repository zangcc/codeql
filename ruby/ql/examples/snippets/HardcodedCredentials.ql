...
select source.getNode().asExpr().getFile().getAbsolutePath()+"$$"+source.getNode().asExpr().getFile().getLocation().getStartLine(),source.getNode(), source, sink, "This hardcoded value is $@.", sink.getNode(),
  "used as credentials"
